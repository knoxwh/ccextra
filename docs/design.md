...
转换逻辑、认证、prompt cache key、thinking 映射、reasoning 回放对齐成熟协议网关实现;九模块归一化对齐字节稳定化 sidecar 的缓存稳定化管线。gemini/antigravity 路径对齐 CLIProxyAPI 的翻译器与 executor(schema 清洗、VALIDATED、maxOutputTokens 封顶、信封与 UA)。