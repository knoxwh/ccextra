...
use ccextra_core::normalize::{
    normalize_anthropic_full, normalize_anthropic_pretransform, normalize_target_post,
    TargetShape,
};
...
fn check_secret(headers: &HeaderMap, secret: &Option<String>) -> Result<(), AppError> {
    ...
    if ok {
        ...
        Err(AppError::unauthorized("x-api-key 或 Authorization 缺失/不匹配"))
    }
}
...
async fn handle_messages(
    ...
    // 5. payload 参数覆盖(转换后注入,协议感知;claude 直通默认不注入)
...
fn to_anthropic_error(body: &[u8]) -> Vec<u8> {
    let (raw_type, raw_message) = serde_json::from_slice::<Value>(body)
        ...
        .and_then(|v| {
            let err = v.get("error")?;
            Some((
                err.get("type").and_then(|x| x.as_str()).unwrap_or("").to_string(),
                err.get("message").and_then(|x| x.as_str()).unwrap_or("").to_string(),
            ))
        })
        .unwrap_or_default();
    let err_type = match raw_type.as_str() {
        t @ ("invalid_request_error" | "authentication_error" | "permission_error"
        | "not_found_error" | "rate_limit_error" | "overloaded_error") => t.to_string(),
...
mod tests {
    ...
    fn test_claude_relay_identity_headers_passthrough_only() {
        ...
        let body = json!({});
        let out = claude_relay_headers(&headers, &body);
        assert!(out.iter().any(|(k, v)| k == "anthropic-version" && v == "2023-06-01"));
        assert!(out.iter().any(|(k, v)| k == "x-app" && v == "cli"));
        assert!(out.iter().any(|(k, v)| k == "x-stainless-os" && v == "macOS"));
        // 未提供的头不出现
