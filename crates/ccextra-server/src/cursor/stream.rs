...
mod tests {
    ...
    #[tokio::test]
    async fn http_proxy_decodes_basic_credentials_before_connect_tunnel() {
        let target = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let target_addr = target.local_addr().unwrap();
        let proxy = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let proxy_url = format!("http://bob%20a:pw%2F1@{}", proxy.local_addr().unwrap());
        let base_url = format!("http://{target_addr}");
        let (done_tx, done_rx) = tokio::sync::oneshot::channel::<()>();
        ...
        let proxy_task = tokio::spawn(async move {
            ...
            assert!(headers.starts_with(&format!("CONNECT {target_addr} HTTP/1.1\r\n")));
            assert!(headers.contains("Proxy-Authorization: Basic Ym9iIGE6cHcvMQ==\r\n"));
        ...
        let run = async {
            let mut stream = CursorStream::open(
                &base_url,
                "access",
                "cli-test",
                "request-1",
